...
class LmdbLoader(LoadImagesAndLabels):
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        for im_file in self.im_files:
            img, json = self.lmdb.read_image_json_pair(im_file)
        ...
        x['msgs'] = []  # warnings
        x['version'] = self.cache_version  # cache version
        LOGGER.info(f'{prefix}Wow, your labels are perfect!')
        try:
            np.save(path, x)  # save cache for next time
            path.with_suffix('.cache.npy').rename(path)  # remove .npy suffix
            LOGGER.info(f'{prefix}New cache created: {path}')
    ...
    def load_image(self, i):
        ...
        if im is None:  # not cached in RAM
            ...
            # --- START OF MODIFIED CODE ---
                ...
                im, json = self.lmdb.read_image_json_pair(key)
    ...
    def cache_images_to_disk(self, i):
        ...
        f = self.npy_files[i]
        if not f.exists():
            img, _ = self.lmdb.read_image_json_pair(self.im_files[i])
            np.save(f, img)
